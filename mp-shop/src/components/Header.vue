		...
		<div class="lg:container mx-auto">
			<div class="flex flex-col lg:flex-row lg:gap-12 relative bg-white">
				<div class="flex lg:w-1/4 justify-between">
					<router-link
						to="/"
						class="max-sm:sticky max-sm:top-0 bg-white p-4 py-6 lg:py-8"
					...
					<button
						...
						class="bg-white p-4 py-6 lg:py-8 lg:hidden"
					>
						<svg
							class="w-8 h-8"
							fill="none"
				...
				<div
					v-if="$route.name"
					role="navigation"
					class="grow lg:w-3/4 p-4 py-6 lg:py-8 hidden lg:block"
				...
				>
					...
					>
						<div class="flex flex-col lg:flex-row gap-4 lg:gap-8">
							<HeaderNavLink
								label="Notizhefte"
								:path="`/notebooks/${bookletCoverId}`"
								@click="trackEvent(`header-booklets-clicked`)"
							/>
							<HeaderNavLink
								label="Notizbücher"
								:path="`/notebooks/${softCoverId}`"
