<template>
	...
	>
		<main class="pt-12 px-4 lg:container mx-auto">
			<div v-if="product">
				...
				<p class="text-gray-800 text-lg lg:w-4/5">
				...
				<div class="flex flex-col-reverse lg:flex-row gap-12 relative">
					<div class="lg:w-1/2 xl:w-2/5 pt-8">
						<div class="rounded-lg bg-white shadow-xl mb-16">
							<section
								class="flex flex-col xl:flex-row gap-4 xl:gap-12 py-6 px-4 xl:px-8"
							>
								<h2
									class="pt-3 xl:w-1/3 uppercase text-gray-500 text-sm font-semibold"
								>
									Einband
								</h2>
								<div class="flex xl:w-2/3 gap-4">
									<SelectionBox
										...
										:path="
											cover.productId &&
											`/details/${cover.productId}`
										"
										:is-active="cover.productId === product.id"
										@click="
											trackEvent(`product-change-cover-clicked`, {
												product: product.id,
												label: cover.name
											})
										"
							...
							>
								...
								<div class="flex xl:w-2/3 gap-4">
									<SelectionBox
										...
										:path="
											ruling.productId &&
											`/details/${ruling.productId}`
										"
										:is-active="ruling.productId === product.id"
										@click="
											trackEvent(`product-change-ruling-clicked`, {
												product: product.id,
												label: ruling.name
											})
										"
							...
							>
								...
								<div class="flex xl:w-2/3 gap-4">
									<SelectionBox
										...
										:path="
											pages.productId &&
											`/details/${pages.productId}`
										"
										:is-active="pages.productId === product.id"
										@click="
											trackEvent(`product-change-pages-clicked`, {
												product: product.id,
												label: pages.name
											})
										"
							...
							<section class="flex gap-12 py-6 px-4 lg:px-8 items-center">
								...
								>
									...
									<span
										itemprop="price"
										class="oldstyle-nums text-3xl tracking-tight font-bold"
									>
						...
						<!-- CopyText Details -->
						<div
							v-if="product.cover?.copyText"
							class="my-16 px-4 flex flex-col gap-y-6"
						>
							<div v-html="product.cover.copyText.text"></div>
							<p>
								<span class="font-bold">Einband:</span>
								{{ product.cover.copyText.cover }}
							</p>
							<p>
								<span class="font-bold">Papier:</span>
								{{ product.cover.copyText.paper }}
							</p>
							<p>
								<span class="font-bold">Format:</span>
								{{ product.cover.copyText.format }}
							</p>
							<p>
								<span class="font-bold">Banderole:</span>
								{{ product.cover.copyText.banderole }}
							</p>
						</div>
					</div>
					<div class="flex justify-center lg:w-1/2 xl:w-3/5 pt-8 mx-auto">
						...
						<div class="w-2/5">
							<VueMagnifier
								...
								:alt="product.name"
								class-name="w-full object-cover"
								itemprop="image"
							/>
						</div>
					</div>
	...
	<section
		class="flex flex-col gap-8 px-4 lg:container mx-auto items-center my-10"
		v-if="patternVariants.patterns"
	>
		<div class="">
			<Title :level="1" html-tag="h2"
				>Erhältlich in {{ patternVariants.patterns.length }} weiteren
				Mustern</Title
			>
		</div>
		...
		<ul class="grid grid-cols-1 lg:grid-cols-5 gap-12">
			<li
				v-for="(pattern, i) in patternVariants.patterns"
				:key="i"
				@click="
					trackEvent(`product-details-pattern-clicked`, {
						product: product.id,
						pattern: pattern.id
					})
				"
			>
				<a
					:href="
						pattern.productVariant?.id
							? `/details/${pattern.productVariant.id}`
							: ``
					"
	...
	methods: {
		...
		async addToCart() {
			...
			trackEvent(`product-details-add-to-cart-clicked`, {
				...
				price: this.product.totalProductPrice
			});
