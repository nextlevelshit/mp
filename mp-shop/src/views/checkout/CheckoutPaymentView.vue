<template>
	<main class="pt-4 lg:container mx-auto px-4">
		...
		<div v-if="cart">
			<div
				class="mt-8 mb-12 flex flex-col-reverse lg:flex-row gap-8 mx-auto relative"
			>
				<div class="lg:w-1/2">
					<div
						v-if="!hasPaymentError"
						class="payment sticky top-4"
						ref="payment"
					></div>
				...
				<div class="lg:w-1/2">
					<div v-if="cartProducts" class="flex flex-col gap-8">
						...
						<div>
							...
							<div class="flex justify-between mb-2">
								...
								<span class="text-2xl">{{
									cart.delivery?.price
										? numberFormatter(cart.delivery.price, "€")
										: "KOSTENFREI"
								}}</span>
							</div>
							<div class="flex justify-between mb-2">
					...
					<div class="bg-gray-100 mt-10 p-8 text-gray-700 rounded-lg relative">
						...
						<a
							...
							@click="
								trackEvent(
									`checkout-payment-change-email-address-clicked`
								)
							"
							class="absolute bottom-8 right-12 hover:underline underline-offset-2"
							>Ändern</a
						>
					</div>
					...
					<div class="bg-gray-100 mt-10 p-8 text-gray-700 rounded-lg relative">
						...
						<p
							...
							v-html="cart.invoiceAddress"
						/>
						<a
							href="/checkout/2"
							@click="
								trackEvent(
									`checkout-payment-change-invoice-address-clicked`
								)
							"
							class="absolute bottom-8 right-12 hover:underline underline-offset-2"
							>Ändern</a
					...
					<div
						v-if="cart.address"
						class="bg-gray-100 mt-10 p-8 text-gray-700 rounded-lg relative"
					>
						...
						<a
							...
							@click="
								trackEvent(
									`checkout-payment-change-delivery-address-clicked`
								)
							"
							class="absolute bottom-8 right-12 hover:underline underline-offset-2"
							>Ändern</a
					...
					<div class="bg-gray-100 mt-10 p-8 text-gray-700 rounded-lg">
						<div>
							...
							<p class="text-sm mt-1">
								Möglicherweise werden Sie im nächsten Schritt von Ihrer
								Bank gebeten, Ihre Kreditkarte zu verifizieren (3-D
								Secure).
							</p>
							<Title :level="4">Hinweise zum Datenschutz</Title>
							<p class="mt-1 text-sm">
								Die personenbezogenen Daten werden für die Abwicklung der
								Bestellung automatisiert verarbeitet. Der Schutz Ihrer
								persönlichen Daten ist uns wichtig. Daher verwenden wir
								bei der Übertragung moderne Verschlüsselungstechnologien.
								Weiteres entnehmen Sie bitte unseren
...
export default {
	...
	methods: {
		...
		async startCheckout() {
			...
			try {
				const { session, clientKey } = await shopApi.checkoutOrder(
					this.cart.uuid
				);
				...
				paymentMethodsConfiguration: {
					card: {
						...
						amount: {
							...
							value:
								session.amount.value || Math.round(this.cart.total * 100)
						}
					},
					paypal: {
						amount: {
