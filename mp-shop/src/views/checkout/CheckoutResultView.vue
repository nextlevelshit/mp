		...
		<div class="flex gap-12 relative max-w-screen-lg mx-auto">
			<div v-if="order" class="flex flex-col gap-8 w-full">
				...
				<p class="text-lg lg:w-2/3">
					In Kürze erhälst du von uns eine E-Mail mit allen Einzelheiten zu
					deiner Bestellung. Du kannst sie auch hier herunterladen, sobald sie
					erstellt wurde.
				</p>
				...
				<div>
					<Button
						:is-pending="
							!hasReachedMaxFailedRequests &&
							(!isReadyToDownload || isDownloadPending)
						"
					...
					<div v-if="hasReachedMaxFailedRequests" class="text-sm mt-3">
						Es konnte noch keine Rechnung ermittelt werden. Bitte prüfe deine
						E-Mails oder kontaktiere uns: order@muellerprints.de
					</div>
				</div>
				...
				<div class="flex flex-col gap-8">
					<ul class="divide-y divide-gray-300">
						<li
							v-for="(position, index) in orderProducts"
							:key="index"
	...
	async mounted() {
		const fetchAndSetOrder = async () => {
			...
			if (this.failedDownloadRequests < maxFailedRequests) {
				...
				throw Error(
					`Failed to retrieve invoice after ${maxFailedRequests} attempts`
				);
			}
		};
		try {
